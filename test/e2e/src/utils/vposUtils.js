...
export const configurePgsMockDev = async (step0Outcome, step1Outcome, step2Outcome) => {
    ...
    let mockConfigurationStatusCode = await configureMockStepDirectAuth(step0Outcome, step1Outcome, step2Outcome);
    ...
    if (mockConfigurationStatusCode === 200) {
      let response = await authRequestVpos();
      if(response.errorHttpStatus===undefined){
        authRequestId = response.requestId;
      }else{
  ...
  export const configureMockStepDirectAuth = async (step0Outcome, step1Outcome, step2Outcome) => fetch(VPOS_MOCK_CONFIGURATION_URL, {
    method: "POST",
    body: JSON.stringify({
      "method3dsOutcome": "OK",
      "step0Outcome": step0Outcome,
  ...
  export const authRequestVpos = async () => fetch(VPOS_AUTH_URL, {
    method: "POST",
    body: JSON.stringify({
